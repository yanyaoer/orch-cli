...
or a `main`/`master`/`trunk` fallback. Teardown: `orch worktree remove
--dest <dest>` — loss detection gates it (kept clones exit 1 with the
`--force` command), and the clone's branch is deleted only when merged.
Leftovers accumulate silently (removal is fail-closed): `orch worktree gc`
inventories this repo's clones and trash and `--execute` removes the
proven-safe ones — the bare `orch` overview nudges when clones sit
unremoved for a week.
