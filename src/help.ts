...
export function worktreeCloneHelp(): string {
  return lines([
    ...
    "                     upstream is set to origin/main when that ref exists.",
    "                     Omitted: the clone stays detached at that commit",
    "  --help             Show this help",
    "",
    "Behavior:",
    "  A CoW copy (probed: APFS `cp -c`, then reflink `cp --reflink=always` for Btrfs/XFS/ZFS)",
    "  clones the whole worktree in seconds at near-zero disk cost, untracked build output",
    "  included — incremental builds in the clone start warm. Filesystems without CoW are",
    "  refused, never silently full-copied. Avoid cloning while a build is writing.",
...
export function fanoutHelp(): string {
  return lines([
    ...
    "                            under /tmp/orch-clones (needs a CoW filesystem: APFS or reflink;",
    ...
    "                            carries the teardown command (/tmp self-cleans regardless)",
