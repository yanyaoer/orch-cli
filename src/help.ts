...
export function worktreeCloneHelp(): string {
  return lines([
    ...
    "",
    "Behavior:",
    "  CoW support is probed in the destination parent: APFS `cp -c`, then Linux reflink",
    "  `cp --reflink=always`. Filesystems without CoW are refused, never silently full-copied.",
    "  The final destination is registered before materialization, so .git/.jj are never copied",
    "  and the clone has its own index/HEAD. Snapshot mode carries uncommitted source edits and",
    "  build output; warm-head starts clean with only configured ignored caches.",
