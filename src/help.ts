...
export function runCreateHelp(): string {
  return lines([
    ...
    "                            falls back to zenmux/anthropic/claude-fable-5 then openai-codex/gpt-5.5",
    ...
    "  Researcher (architect) is claude/codex-only, read-only, and web-research capable:",
    "  claude runs fable at xhigh effort with a whitelist (jina/tvly CLIs + web tools);",
    "  codex defaults to gpt-5.6-sol at xhigh reasoning with native web search on.",
    "  It delivers a recommendation (orch.result/researcher/v1), never code.",
  ...
  switch (topic) {
    case "task-spec":
      return lines([
        ...
        "    (omp = oh-my-pi: gemini-3.1-pro by default with quota fallback to claude-fable-5, then gpt-5.5)",
...
export function fanoutHelp(): string {
  return lines([
    ...
    "  investigate    Reviewer role, read-only research; default agents: omp-reviewer, claude-reviewer",
    "",
    "Flags:",
