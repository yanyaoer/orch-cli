...
export function runCreateHelp(): string {
  return lines([
    ...
    "                            agy takes the prompt via argv (ps-visible, task capped at 512KB)",
    "  Read-only roles are sandboxed per provider: reviewer launches the worker without",
    "  write access (claude plan mode / codex read-only / pi read tools / agy --sandbox).",
  ...
  switch (topic) {
    case "task-spec":
      return lines([
        ...
        "  - Agent: codex, claude, pi, or agy, the headless provider driver that will execute the task",
        "    (agy = gemini-3.1-pro, reviewer role only, sandboxed read-only)",
...
export function fanoutHelp(): string {
  return lines([
    ...
    "",
    "Commands:",
    "  cross-review   Reviewer role; default agents: claude-reviewer, agy-reviewer",
    "  fanout         Any result role via --role (required); agents auto-invited for that role",
    "  investigate    Reviewer role, read-only research; default agents: agy-reviewer, claude-reviewer",
    "",
    "Flags:",
