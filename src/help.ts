...
export function newHelp(): string {
  return lines([
    ...
    "",
    "Flow:",
    "  1. A read-only researcher run (claude fable/xhigh) drafts the plan:",
    "     goal, task breakdown, acceptance, and numbered open questions with",
    "     recommended answers.",
    "  2. The plan prints in your terminal. Press Enter to accept, type answers or",
    "     amendments to replan (repeatable), or q to abort (plan runs are kept).",
    "  3. The same provider session resumes as a controller (Bash `orch *` only,",
    "     no code edits) that dispatches workers via fanout/cross-review/run create",
    "     with inline `--task -` heredocs, waits, and records decisions.",
    "",
    "Flags:",
    ...
    "  --model <ref>             Model override for the plan run (exec resumes the same session)",
    "  --timeout-sec <seconds>   Timeout for the plan run",
    "  --yes                     Non-interactive: accept the recommended plan without confirmation",
    "  --help                    Show this help",
    "",
    "Output: progress and the plan render on stderr; the final JSON payload",
    "(mr, plan_runs, exec_run, follow_up commands) prints on stdout.",
    "",
    "Examples:",
