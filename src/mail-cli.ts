...
export function defaultMailAgents(now: string): MailAgentDefinition[] {
  return [
    ...
    {
      id: "codex-researcher",
      address: "orch+codex.researcher@local.orch",
      provider: "codex",
      roles: ["researcher"],
      capabilities: ["research", "architecture", "web-search"],
      trust: "internal",
      auto_invite: true,
      work_mode: "research",
      provider_session_mode: "fresh_persistent",
    ...
    },
    {
      ...
      address: "orch+claude.researcher@local.orch",
      provider: "claude",
      roles: ["researcher"],
      capabilities: ["research", "architecture", "web-search", "long-context"],
      trust: "internal",
      auto_invite: true,
      work_mode: "research",
      provider_session_mode: "fresh_persistent",
...
function findingLine(finding: Record<string, unknown>): string {
  const head = [finding.severity, finding.id, finding.file].filter((part) => typeof part === "string" && part).join(" | ");
  const raw = typeof finding.body === "string" ? finding.body : typeof finding.summary === "string" ? finding.summary : "";
  const text = raw.replace(/\s+/g, " ").trim();
  const clipped = text.length > 220 ? `${text.slice(0, 220)}...` : text;
  return `  - [blocking${head ? ` | ${head}` : ""}] ${clipped}`;
}

// One fan-out dispatch shares the timestamp segment embedded in its run ids
// (mail-<role>-<agent>-<TS>-<hex>): that segment is the round key.
function reviewRoundOf(runId: string): string {
  return runId.split("-").at(-2) ?? runId;
}

// --rework: the mechanical part of a re-review brief, generated from run
// state so the operator does not hand-copy it each round — prior rounds'
...
export function buildReworkAppendix(repoKey: string, mr: string, currentHead: string | null): string {
  ...
  const rounds = [...new Set(priors.map((prior) => reviewRoundOf(prior.run_id)))].sort();
  const lastReviewed =
    priors
      .filter((prior) => prior.base_sha)
  ...
  const lines: string[] = [
    ...
    "---",
    "",
    "## Prior review rounds (auto-generated by --rework)",
    "",
    `This is round ${rounds.length + 1}. The findings below were already adjudicated;`,
    "re-raise one only with NEW evidence beyond the recorded reasoning.",
  ];
  if (lastReviewed && currentHead && lastReviewed !== currentHead) {
    lines.push("", `REVIEW SCOPE — review ONLY: git diff ${lastReviewed}...${currentHead}`);
  } else if (lastReviewed && currentHead) {
    lines.push(
      "",
      `NOTE: HEAD equals the last reviewed commit (${currentHead}); there is nothing new to review unless history was rewritten.`,
    );
  }
  if (rounds.length >= 3) {
    lines.push(
      "",
      `WARNING: ${rounds.length} prior rounds already ran. If a blocker repeats without new evidence, say so and stop instead of re-litigating.`,
    );
    process.stderr.write(`orch: warning: --rework is dispatching round ${rounds.length + 1}; consider stopping if blockers repeat\n`);
  }
  for (const [index, round] of rounds.entries()) {
    lines.push("", `### Round ${index + 1}`);
    const members = priors
      .filter((prior) => reviewRoundOf(prior.run_id) === round)
      .sort((a, b) => a.agent.localeCompare(b.agent) || a.run_id.localeCompare(b.run_id));
    for (const prior of members) {
      lines.push(
        `- ${prior.agent} (${prior.run_id}): state=${prior.state}, verdict=${prior.verdict ?? "-"}, decision=${prior.decision ?? "undecided"}`,
      );
      for (const finding of prior.blocking) lines.push(findingLine(finding));
      if (prior.non_blocking_count > 0) lines.push(`  - non-blocking: ${prior.non_blocking_count} recorded finding(s)`);
    }
  }
  lines.push("");
  return lines.join("\n");
...
export async function mailFanout(args: ParsedArgs, context: MailCliContext, opts: MailFanoutOptions): Promise<MailFanoutOutcome> {
  ...
  const rawTaskText = taskFlagValue === "-" ? await readStdinText() : readFileSync(resolve(taskFlagValue), "utf8");
  if (taskFlagValue === "-" && !rawTaskText.trim()) throw new CliError("--task - received empty stdin");
  const workspace = workspaceForMail(args.flags.has("workspace") ? flagString(args, "workspace") : undefined);
  ...
  // --rework appends the mechanically derivable part of a re-review brief;
  // the changed task text also changes task_sha, so a fresh round is
  // published instead of reusing round-1 events.
  const taskText = flagBool(args, "rework")
    ? rawTaskText + buildReworkAppendix(repo.repo_key, mr ?? thread, clone?.head ?? (await vcsHead(worktree)))
    : rawTaskText;
