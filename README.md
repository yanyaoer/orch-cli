...
- `orch mailctl` drives orch by real email over IMAP/SMTP: an allowlisted, authenticated sender emails a task, `orch mailctl poll` ingests it, auto-spawns a claude **controller** run that decomposes/dispatches the work and replies progress in the same thread. Daemonless (`poll` is the cron/launchd contract; `watch` is a foreground IMAP-IDLE convenience), zero npm deps, any IMAP/SMTP provider (Gmail via app password). Inbound email is treated as an authentication boundary (allowlist + trusted-authserv-id Authentication-Results, fail-closed; text/plain-only task body). The controller result schema is `orch.result/controller/v1`. Replies are version-guarded (**held drafts**): every accepted inbound message and delivered reply bumps a per-thread version, and a reply submitted with a stale `--base-version` is parked with the events it missed until explicitly revised, released, or withdrawn — no silent stale sends, no double-answers between concurrent agents.
...
**Maildir transport (recommended).** The built-in IMAP/SMTP client is the one place orch holds a network socket, and every mail incident so far (a half-open IMAP connection holding the ingest lock for hours, SMTP submissions failing and replies dropped after eight retries) lived there. With `"transport": {"kind": "maildir", ...}` an external fetcher owns IMAP and an external submitter owns SMTP; orch reads and writes a directory. `sync_cmd` runs before every listing, so a scheduled `poll` stays self-contained; without `send_cmd` the built-in SMTP client still sends. mbsync writes its own per-store UID into the filename (`,U=<n>`, monotonic for that Maildir, not the server UID), which keeps the poll cursor monotonic — when pointing orch at a new Maildir, drain the old mailbox with a last poll, then set `cursor.json`'s `last_uid` to the new store's highest `U=` before polling it: a cursor left above the store's range skips every message forever (polls report only duplicates, no error, no alert), one left below re-ingests everything above it; processed mail moves to `cur/` as seen, which a two-way sync propagates to the server. There is no push channel: latency is the poll/watch interval unless an IMAP IDLE notifier (e.g. `goimapnotify`) triggers `orch mailctl poll`.
...
- `native.jsonl`: provider-native stream, not treated as orch events; `orch events tail --native` renders it as normalized progress events. pi/omp `message_update` and `tool_execution_update` streaming deltas (accumulated partial snapshots that no reader consumes) are dropped at write time; complete messages stay in `message_end`, complete tool results in `tool_execution_end`
...
The driver prompt asks the provider to return exactly one JSON object and prints the exact shape to copy (`ROLE_RESULT_EXAMPLE` in `src/schema.ts`, the same source the validator enforces). The driver extracts that object and repairs only deviations with a single reading: verdict case and known synonyms, arrays the model left out, `*_run_id` typed as numbers, `run_id`/`base_sha` that the spec owns, a bare string as a non-blocking finding body, and a gemini-style single `findings` list. Anything else — a finding body under another key, a blocking finding without id/severity/file, object items where strings belong, a missing summary — is rejected and, when the provider exited normally, sent back to the model once with the validator's own errors and its previous answer (the repair round; `result_repair` in `events.jsonl`, the repair stream appended to `native.jsonl` after an `orch_result_repair` marker; `ORCH_RESULT_REPAIR=0` disables it). A result that still does not validate fails the run with the raw answer preserved in `result.raw.md`.
...
orch worktree clone  CoW-clone a worktree into an isolated per-agent checkout (APFS clonefile/reflink,
                   carries untracked build output so incremental builds start warm)
orch worktree remove  Remove one clone behind the loss-detection gate (--force discards; merged branch tidied)
orch worktree gc   Inventory this repo's clones and trash; remove the proven-safe ones with --execute
