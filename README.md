...
- **P0**：工程骨架 + 状态目录/`repo_key`/`run_id` + status/result/events 的 TS 类型 + 3 个 role result schema（`schemas/`）；**验证两个 posix 命脉点**——进程组 kill（`setsid`+`kill -pgid`）、`O_EXCL` pidfile 锁 + stale 检测。
