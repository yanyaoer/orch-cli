...
- `orch cross-review`, `orch fanout`, and `orch investigate` fan one task across several agents in a single command. They route through the mail layer, so a `--thread <id>` supplies the mr and workspace context (no `--mr` needed). With `--clone`, all read-only workers share one CoW snapshot of the worktree — the review target can't shift mid-run and there is no `index.lock` contention with your own git.
- `orch worktree clone` CoW-clones a worktree (APFS `cp -c` / reflink) into an isolated per-agent checkout in seconds at near-zero disk cost, untracked build output included, so the clone's incremental builds start warm.
