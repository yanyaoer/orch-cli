...
- `config.json` gains `defaults.agents.<role>`: `orch run create` falls back to it when `--agent` is omitted (explicit flag still wins). Recommended profile: `implementer -> pi`.
